...
void iterativeDFS(const DiGraph &graph, std::stack<Vertex> &vertexStack, std::stack<Vertex> &iteratorStack,
                  std::stack<Vertex> &tarjanStack, boost::dynamic_bitset<> &tarjanVisited,
                  std::vector<Vertex> &discovery, std::vector<uint32_t> &lowLink,
                  std::vector<std::vector<Vertex>> &components, std::vector<uint32_t> &vertexMapping,
                  uint32_t &currentIndex, Vertex start);
...
std::unique_ptr<SCCGraph> tarjanSCC(const DiGraph &graph, bool includeComponents) {
    ...
    for (auto vertex = 0; vertex < graph.getVertexCount(); vertex++) {
        if (discovery[vertex] == 0) {
            ...
            iterativeDFS(graph, vertexStack, iteratorStack, tarjanStack, tarjanVisited, discovery, lowLink, components,
                         vertexMapping, currentIndex, vertex);
        }
    }
    ...
    for (auto sourceComponent = 0; sourceComponent < components.size(); sourceComponent++) {
        for (auto source : components[sourceComponent]) {
            for (auto target : graph.getConnected(source)) {
                auto targetComponent = vertexMapping[target];
        ...
        if (iteratorPointer < adjacencyList.size()) {
            ...
            if (lowLink[vertex] == discovery[vertex]) {
                ...
                do {
                    ...
                    vertexMapping[connectedVertex] = uint32_t(components.size() - 1);
